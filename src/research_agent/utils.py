...
def remove_think_tags(running_summary: str) -> str:
...
def extract_rag_results(rag_results):
    # Split by sections using the markers
    answer_section = re.search(
        r"=+Answer=+\n(.*?)\n=+Source docs=+", rag_results, re.DOTALL
    )
    source_section = re.findall(r"source:\s*(.*?)\nscore:", rag_results, re.DOTALL)

    answer_text: str = (
        answer_section.group(1).strip() if answer_section else "No answer found"
    )

    sources = source_section if source_section else ["No sources found"]
